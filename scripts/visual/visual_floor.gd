...
var counter: int = 0
var camera_current_x: float = 0.0
var distance_moved: float = 0.0

func _ready() -> void:
	floor_tile_width = get_viewport().get_visible_rect().size.x
	Signals.camera_move.connect(on_camera_move)
...
func on_camera_move(camera_pos: Vector2) -> void:
	if camera_current_x == 0.0:
		camera_current_x = camera_pos.x
	distance_moved += (camera_pos.x - camera_current_x)
	camera_current_x = camera_pos.x
	if distance_moved >= floor_tile_width:
		if counter % 2 == 0:
			sprite1.global_position.x = sprite2.global_position.x + floor_tile_width
